...
if($vcmemail != 1) {
	$searchEmail = Account::checkEmailExists($newemail);
...
if(!empty($acc[0]['email']) && $chaemail_confirm == 1) {
	...
	$insertCode = Account::insertEmailCode($newemail, $code, $_SESSION['acc']);
	if(!$insertCode) {
		fim($LANG[12055]);
	} else {
	...
	$updateEmail = Account::updateEmail($newemail, $_SESSION['acc']);
	if(!$updateEmail) {
		fim($LANG[12055]);
	} else {
		fim($LANG[12056], 'OK', './');
