<?php if(!$indexing || $dpage['toplvl'] != 1) { echo "<script>document.location.replace('./');</script>"; exit; } ?>
<h1>Top Level</h1>
...
<a href='./?page=toppvp'>Top PvP</a>
				...
				<a href='./?page=toplevel'class='ativa'>>Top level</a>
...
if($genNew) {
	...
	if($showRankReg == 1 || ($showRankReg == 0 && time() > $dateReg)) {
		...
		$query = Stats::TopLevel($countTopPlayers);
		if(count($query) > 0) {
...
<div class='pddInner'>
	<?php echo $LANG[30512]."<br />".$LANG[30500]." <b>".date('d/m/Y', $updated)."</b> ".$LANG[30501]." <b>".date('H:i', $updated)."</b>."; ?><br /><br />
</div>
